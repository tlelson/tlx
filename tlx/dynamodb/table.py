...
def append_to_list_field(table, key, field_to_update, expression_attribute_names, new_item, add_missing_key=False):
    ...
    return add_new_map_field(
        table, key, field_to_update[:-1], field_to_update[-1], expression_attribute_names, new_item, add_missing_key
    )


def add_new_map_field(
    table, key, path, field_to_add, expression_attribute_names, data, add_missing_key=False, replace_existing=False
):
    ...
    if not path:  # Limit of recursion (Primary key was not found)
        return add_key(table, key, field_to_add) if add_missing_key else {field_to_add: data}
    ...
    return add_new_map_field(
        table, key, path[:-1], path[-1], expression_attribute_names, new_data, replace_existing=replace_existing
    )
